...
- 修复本地后端仍运行旧进程时演示流程页一直停留在“正在加载演示脚本”的问题，改为显示明确的后端重启提示。
- 修复模型状态页在训练 readiness 接口暂不可用时整页失败的问题，保留模型运行状态并单独提示训练接口需重启后端。
- 修复系统总览在旧后端缺少 `data_quality`、`actions` 或测量来源字段时白屏的问题，增加缺字段容错展示。
