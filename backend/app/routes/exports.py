...
def _cohort_row(patient: Patient, nodule: Nodule | None) -> dict[str, Any]:
    ...
    return {
        ...
        "clinical_label": nodule.clinical_label,
        "pathology_label": nodule.pathology_label,
        ...
        rows.append(
            {
                ...
                "clinical_label": nodule.clinical_label if nodule else None,
                "pathology_label": nodule.pathology_label if nodule else None,
...
def _build_report_rows(db: Session, patient_id: int | None = None) -> list[dict[str, Any]]:
    query = db.query(Report).order_by(Report.created_at)
    if patient_id is not None:
...
def _report_export_text(report: Report) -> str:
...
analysis_id: {report.analysis_id}
created_at: {report.created_at}
finalized_at: {report.finalized_at or ''}

{report.content_markdown.strip()}
