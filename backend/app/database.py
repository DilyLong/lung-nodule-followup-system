...
def migrate_sqlite_schema() -> None:
    with engine.begin() as conn:
        ...
        expected_tables = {
            "import_batches": "CREATE TABLE IF NOT EXISTS import_batches (id INTEGER PRIMARY KEY, created_at DATETIME, committed_at DATETIME, rolled_back_at DATETIME, status VARCHAR(32), qc_score FLOAT, counts_json TEXT, issues_json TEXT, message TEXT)",
            "import_batch_entities": "CREATE TABLE IF NOT EXISTS import_batch_entities (id INTEGER PRIMARY KEY, batch_id INTEGER, entity_type VARCHAR(32), entity_id INTEGER, action VARCHAR(32), stable_key VARCHAR(256), previous_json TEXT)",
            "report_versions": "CREATE TABLE IF NOT EXISTS report_versions (id INTEGER PRIMARY KEY, report_id INTEGER, version_number INTEGER, created_at DATETIME, status VARCHAR(32), content_markdown TEXT, doctor_opinion TEXT, followup_plan TEXT)",
            "report_audit_logs": "CREATE TABLE IF NOT EXISTS report_audit_logs (id INTEGER PRIMARY KEY, report_id INTEGER, created_at DATETIME, event VARCHAR(64), message TEXT)",
        }
